import { pipe } from "rambda";
import { readInputFile } from "../../utils/readInput.ts";
import { prepareInputData } from "./util.ts";
import { forEach } from "../../utils/forEach.ts";

const mockInput = [
  "16 17 18 21 23 24 27 24",
  "48 51 53 54 55 59",
  "29 31 32 34 36 39 41 46",
  "9 12 9 11 14 16 17 20",
  "65 68 66 67 69 70 73 72",
  "56 58 59 58 61 64 64",
  "21 24 25 27 24 25 29",
  "83 85 88 91 90 96",
  "68 69 72 75 75 74",
  "34 37 38 38 41 43 43",
  "41 43 44 47 49 49 53",
  "69 71 72 74 74 75 76 83",
  "53 56 57 60 61 65 66",
  "46 49 53 55 53",
  "41 43 45 49 52 52",
  "37 39 43 46 50",
  "51 54 55 56 60 66",
  "30 32 39 42 45",
  "58 59 60 65 64",
  "63 65 68 70 72 74 79 79",
  "50 53 56 62 66",
  "64 66 67 72 74 76 82",
  "12 11 12 15 16 15",
  "63 60 63 65 68 70 70",
  "15 13 16 18 19 22 26",
  "86 83 85 86 89 92 98",
  "73 71 74 75 72 74",
  "68 67 69 67 64",
  "83 81 83 81 81",
  "31 28 30 28 32",
  "88 87 84 85 92",
  "18 16 19 21 22 22 23",
  "19 16 16 17 14",
  "49 46 46 48 48",
  "9 8 11 12 12 13 14 18",
  "23 20 21 21 28",
  "81 78 82 84 83",
  "60 58 61 65 65",
  "62 60 64 65 68 71 72 76",
  "51 49 52 56 62",
  "5 3 5 12 13 14",
  "26 24 26 27 33 36 33",
  "15 14 15 22 22",
  "79 76 77 83 87",
  "40 37 39 44 50",
  "68 68 71 74 76 73",
  "74 74 75 78 78",
  "77 77 79 82 83 84 86 90",
  "39 39 41 44 51",
  "39 39 42 44 47 46 49 51",
  "39 39 42 43 41 44 43",
  "51 51 50 53 55 55",
  "74 74 75 73 77",
  "23 23 22 24 27 34",
  "1 1 3 4 5 5 8",
  "73 73 74 76 77 77 78 77",
  "46 46 49 49 49",
  "63 63 65 67 67 71",
  "21 21 22 22 24 27 30 37",
  "1 1 4 5 9 10 13",
  "74 74 77 81 79",
  "12 12 16 17 17",
  "70 70 73 77 78 80 84",
  "9 9 13 16 17 19 26",
  "19 19 26 27 28 30",
  "41 41 43 49 51 49",
  "84 84 89 92 95 97 97",
  "11 11 14 21 23 24 25 29",
  "80 80 81 84 89 91 96",
  "61 65 67 68 70 73 74 73",
  "31 35 38 40 41 42 42",
  "3 7 10 11 12 15 19",
  "79 83 86 87 88 91 97",
  "88 92 91 94 95 97 96",
  "44 48 46 49 49",
  "51 55 57 60 63 61 65",
  "14 18 19 18 21 24 27 32",
  "62 66 66 67 70 72 75",
  "27 31 31 34 35 36 39 36",
  "68 72 73 73 75 75",
  "56 60 62 65 66 69 69 73",
  "40 44 47 47 54",
  "49 53 57 58 60 61",
  "83 87 88 90 92 96 95",
  "55 59 60 63 65 69 69",
  "18 22 25 29 32 36",
  "14 18 19 23 28",
  "45 49 50 52 59 61 64 65",
  "26 30 35 36 37 40 39",
  "80 84 87 90 96 96",
  "80 84 85 88 95 99",
  "25 29 31 37 42",
  "39 44 46 49 51 53 50",
  "14 19 21 23 24 26 28 28",
  "64 70 72 74 75 77 78 82",
  "20 27 30 33 40",
  "37 42 41 44 46 47",
  "19 25 23 25 28 31 34 31",
  "65 71 70 73 73",
  "71 76 73 76 79 83",
  "46 53 54 57 60 59 64",
  "63 70 71 73 75 78 78 81",
  "64 69 70 71 71 72 73 71",
  "80 87 87 89 90 91 91",
  "48 54 54 56 57 58 62",
  "24 30 32 34 34 37 42",
  "69 75 76 80 83 84 86",
  "42 47 49 53 56 55",
  "68 75 76 80 82 83 84 84",
  "9 14 15 19 23",
  "69 76 80 83 85 91",
  "41 48 51 52 57 58",
  "53 58 61 62 69 72 70",
  "23 30 32 38 38",
  "10 15 18 25 28 32",
  "24 31 32 34 37 42 45 51",
  "25 23 22 20 18 15 12 8",
  "87 85 83 81 79 74",
  "22 19 16 14 11 14 11",
  "5 2 4 3 6",
  "75 72 69 72 69 69",
  "77 75 76 75 74 70",
  "49 46 45 42 40 43 41 34",
  "77 74 73 73 70 69 67 69",
  "36 34 34 32 31 29 28 28",
  "63 61 61 58 56 52",
  "49 47 46 43 41 38 38 32",
  "39 38 34 32 30",
  "11 10 7 6 5 1 1",
  "87 86 85 81 78 75 71",
  "41 39 36 32 29 27 26 19",
  "62 60 58 52 50 47",
  "49 46 43 36 33 30 30",
  "57 55 48 46 44 41 37",
  "87 84 81 80 77 76 70 64",
  "24 25 24 23 21 19 18 17",
  "85 88 86 83 80 77 80",
  "55 57 55 52 51 51",
  "96 97 94 93 92 91 88 84",
  "53 55 53 51 48 43",
  "12 15 16 15 13 10 8",
  "39 42 43 42 39 42",
  "94 96 97 95 94 94",
  "28 30 31 29 25",
  "43 46 47 44 43 38",
  "43 46 44 44 43 42 41",
  "16 18 18 15 14 12 9 10",
  "69 70 70 68 67 66 66",
  "70 71 69 66 65 65 61",
  "24 27 27 24 23 21 16",
  "28 31 30 28 24 22 20 19",
  "84 87 85 83 80 79 75 78",
  "25 26 23 19 17 17",
  "70 71 69 67 66 63 59 55",
  "45 48 44 41 40 35",
  "84 85 79 76 74",
  "47 50 43 40 37 40",
  "39 40 37 34 27 27",
  "43 45 42 41 38 31 27",
  "74 75 73 70 64 57",
  "93 93 90 87 84 81 84",
  "76 76 74 71 70 69 69",
  "43 43 40 37 33",
  "83 83 81 80 78 73",
  "85 85 82 79 80 78",
  "7 7 8 6 3 1 3",
  "32 32 35 34 32 31 31",
  "92 92 93 92 89 85",
  "52 52 50 52 49 43",
  "77 77 75 74 72 72 70 67",
  "32 32 30 29 28 26 26 29",
  "12 12 12 9 6 6",
  "97 97 94 94 91 89 85",
  "30 30 27 27 20",
  "21 21 20 16 13",
  "89 89 86 83 80 76 77",
  "53 53 50 48 44 44",
  "72 72 68 65 61",
  "36 36 35 32 31 28 24 19",
  "39 39 37 31 29 28 25",
  "53 53 47 45 42 43",
  "16 16 9 8 8",
  "42 42 40 39 37 32 28",
  "61 61 59 56 54 48 42",
  "96 92 90 87 86 84 83 84",
  "56 52 50 49 47 47",
  "89 85 84 81 80 76",
  "38 34 33 30 25",
  "50 46 44 45 42 41 38 37",
  "98 94 93 92 93 90 89 92",
  "14 10 8 7 9 9",
  "19 15 18 16 12",
  "43 39 38 39 33",
  "68 64 63 62 59 59 57 54",
  "53 49 48 48 45 46",
  "61 57 55 55 53 51 48 48",
  "48 44 42 42 40 39 37 33",
  "51 47 47 44 37",
  "37 33 30 27 25 21 18",
  "83 79 76 73 69 66 67",
  "37 33 30 26 24 22 21 21",
  "72 68 65 64 63 62 58 54",
  "51 47 44 42 40 36 30",
  "37 33 27 24 22 19 18",
  "19 15 8 7 4 7",
  "80 76 71 68 68",
  "42 38 37 31 27",
  "71 67 66 61 58 57 51",
  "27 22 20 18 17 15 18",
  "40 35 32 31 28 28",
  "32 27 25 24 23 19",
  "65 58 55 54 53 51 50 44",
  "45 39 36 38 37",
  "56 51 49 50 51",
  "45 40 37 34 31 34 32 32",
  "96 91 90 92 89 85",
  "46 40 41 38 36 33 28",
  "73 67 67 65 64 62 60",
  "32 25 22 19 19 18 20",
  "31 26 24 23 20 20 20",
  "72 66 66 63 60 59 55",
  "39 34 32 30 27 27 26 19",
  "43 36 32 31 29 28",
  "73 66 62 60 58 55 57",
  "37 30 29 25 25",
  "83 76 72 70 67 63",
  "62 57 55 52 48 41",
  "86 80 73 70 67",
  "67 60 58 57 54 47 45 48",
  "73 67 62 59 57 55 55",
  "85 79 78 73 69",
  "91 86 85 84 78 75 69",
  "48 42 40 37 32 30 30",
  "18 24 26 28 26 26",
  "84 87 86 85 84 86 84 84",
  "70 77 80 81 78 84",
  "53 46 43 42 38 38",
  "59 64 67 69 71 76 77 77",
  "69 68 64 63 60 62",
  "85 82 81 81 78 78",
  "31 27 21 18 18",
  "70 71 70 67 70",
  "95 93 90 87 84 77",
  "68 72 76 78 79 81 82 88",
  "98 98 96 96 96",
  "15 8 5 4 4",
  "68 69 66 69 73",
  "97 93 95 92 89 86 89",
  "71 67 66 65 62 57 58",
  "55 55 53 49 46 45 43 41",
  "66 63 60 57 55 51 47",
  "81 84 80 79 81",
  "11 15 17 20 23 29",
  "56 60 63 64 71 73 75 79",
  "85 86 79 76 76",
  "40 40 38 38 37 40",
  "35 31 30 30 28",
  "63 67 70 73 77 77",
  "79 86 87 89 95 94",
  "12 16 19 20 21 21 22 24",
  "28 28 25 22 20 18 16 16",
  "45 50 52 53 53",
  "37 41 44 49 47",
  "2 4 5 9 11",
  "53 53 52 49 44 42 41 42",
  "50 50 54 57 60 65",
  "35 42 46 48 49 56",
  "44 45 50 51 54 52",
  "33 29 27 25 22 21 17",
  "18 13 12 12 5",
  "24 19 17 10 7 5 2 1",
  "54 58 61 66 68 70 73",
  "86 86 88 89 88 85",
  "84 84 87 85 83",
  "59 55 53 50 48 47 40",
  "8 9 10 10 11 17",
  "46 46 49 47 48 51 57",
  "85 81 78 77 72 70 66",
  "70 76 79 85 86",
  "58 54 51 48 45 38 36 31",
  "40 40 41 41 42 46",
  "73 70 68 67 62 61 58 56",
  "14 17 17 14 13 11 11",
  "6 3 4 7 11 14 17 18",
  "81 83 82 78 76 73 68",
  "86 89 89 87 85 87",
  "23 24 23 16 15",
  "36 37 36 29 25",
  "31 31 28 25 19",
  "88 88 87 88 87 85 88",
  "69 70 67 64 61 60 57 50",
  "43 42 44 50 50",
  "25 31 32 36 34",
  "18 18 24 25 25",
  "42 42 43 48 47",
  "41 43 40 43 44 45 45",
  "88 90 88 86 84 82 80",
  "77 76 73 71 65 65",
  "30 33 36 39 40 40 41 45",
  "5 5 12 15 19",
  "86 86 88 90 90",
  "84 84 82 81 79 75 68",
  "8 8 7 7 5",
  "75 79 81 83 86 90",
  "24 23 25 27 26 31",
  "51 55 57 60 60 58",
  "81 78 76 73 68 66 65 60",
  "51 58 59 60 62 62 63 70",
  "72 72 70 71 75",
  "53 49 48 41 39 37",
  "13 15 13 16 12",
  "70 74 75 79 78",
  "95 91 88 86 84 84 82 78",
  "70 64 63 61 58 57 53 47",
  "66 60 59 61 60",
  "42 42 39 33 32 29 25",
  "1 8 11 18 20 25",
  "4 4 7 10 9 11 14 14",
  "93 87 83 80 76",
  "68 75 76 78 77",
  "23 18 15 13 11 7",
  "71 67 64 64 63 60 58 51",
  "85 79 77 76 73 72 65",
  "50 48 49 52 52 57",
  "23 19 15 12 10 7 4 5",
  "93 95 93 91 93 90 89 90",
  "30 25 24 22 25",
  "93 96 94 92 91 87 83",
  "74 78 80 79 81 83 84 84",
  "67 65 66 68 74 76 82",
  "60 58 51 50 46",
  "30 26 23 22 21 18 21",
  "33 30 28 26 23 21 19 22",
  "17 16 17 19 26 25",
  "68 66 62 61 58 56 53 53",
  "19 15 13 11 11 11",
  "64 65 64 61 58 57 55 55",
  "27 26 26 28 31 34",
  "22 26 28 30 33 36 35 37",
  "51 51 55 56 59",
  "65 59 56 58 55 55",
  "42 38 36 34 34 37",
  "94 91 89 91 90 89 87 83",
  "36 32 30 26 24 22 19 16",
  "16 12 10 6 6",
  "53 53 56 58 60 59",
  "65 69 66 69 71 73 71",
  "34 27 24 17 16 17",
  "30 31 30 32 29 26",
  "6 13 13 15 17 21",
  "57 50 46 44 45",
  "86 83 81 83 87",
  "94 94 91 90 86 83 83",
  "42 42 43 41 40 40",
  "28 32 35 35 38 40 41 41",
  "3 3 5 8 11 13 14 19",
  "62 65 64 62 62 60 59",
  "84 87 80 79 78 72",
  "27 29 30 32 35 38 44",
  "49 49 48 47 45 44 42 38",
  "54 51 50 52 55 58 60 60",
  "30 36 40 43 44",
  "26 25 26 30 32 35 35",
  "16 19 21 23 26 29 33",
  "45 45 42 39 38 37 35 37",
  "20 19 16 13 12 9 5 3",
  "88 87 87 86 81",
  "16 15 18 21 28 32",
  "57 62 66 68 70 73 77",
  "10 14 17 23 23",
  "92 89 93 95 96 95",
  "51 51 57 59 61 67",
  "8 8 11 15 16 15",
  "20 23 24 27 24",
  "16 22 22 25 26 29 26",
  "71 74 75 78 85 87 90 91",
  "90 85 88 85 81",
  "35 33 36 43 44 47",
  "81 79 80 80 83 87",
  "31 31 31 29 25",
  "14 21 22 22 24 27 28 28",
  "62 66 67 69 69 76",
  "20 18 21 22 25 29",
  "76 74 72 67 64 65",
  "39 37 36 33 32 28",
  "20 27 28 29 32 36 37 37",
  "42 41 39 36 37 32",
];

const test = 1;
const input = test ? await readInputFile(Deno.cwd()) : [
  // "7 6 4 2 1",
  // "1 2 7 8 9",
  // "9 7 6 2 1",
  // "1 3 2 4 5",
  // "8 6 4 4 1",
  // "1 3 6 7 9",
  // ...mockInput,
  // "42 41 39 36 37 32",
  // "20 18 21 22 25 29",
  // "71 74 75 78 85 87 90 91",
  // "45 45 42 39 38 37 35 37",
  // "39 37 36 33 32 28",
  // "16 19 21 23 26 29 33",
  // "11 9 6 5 3 3",
];

const result = pipe(
  prepareInputData,
  function validateData(input) {
    const result: number[] = [];
    let notSafeCount = 0;

    for (const row of input) {
      const data = prepareSequence(row);
      const isValid = data.prep.every((val) => val === 1);

      if (isValid) {
        result.push(1);
        continue;
      }

      // const shouldTry = data.prep.filter((val) => val === 0).length === 1;
      // const shouldTry = false;

      // if (!shouldTry) {
      //   continue;
      // }

      console.log({
        row: row.join(" "),
        prep: data.prep.join(" "),
      });

      // console.log({
      //   row: row.join(" "),
      //   result: data.result.join(" "),
      //   prep: data.prep.join(" "),
      // });

      let isSafe = false;

      forEach(data.prep, (val, i) => {
        if (val === 1) return;

        let isValid = false;

        forEach([i, i + 1], (x, index2) => {
          const fixedRow = row.toSpliced(x, 1);
          const fixedData = prepareSequence(fixedRow);

          isValid = fixedData.prep.every((val) => val === 1);

          console.log(index2, x, {
            fixedRow: fixedRow.join(" "),
            result: fixedData.result.join(" "),
          });

          if (isValid) {
            console.log("isValid");
            isSafe = true;
            result.push(1);
            return false;
            // data.prep = data.prep.slice(); // break the loop
            // indexes.arr = indexes.arr.slice(); // break the loop
          }
        });

        if (isValid) {
          return false;
        }
      });

      if (!isSafe) {
        notSafeCount++;
        // console.log(row.join(" "));
      }

      console.log("---------------------------");
    }

    // console.log({ notSafeCount, prev: 387 });

    return result;
  },
)(input);

console.log({
  finally: "Tada",
  // result,
  sum: result.reduce((acc, val) => acc + val, 0),
});

function prepareSequence(data: number[]) {
  const direction = data[0] - data[1];

  const result: number[] = [];
  let prev: number;

  data.forEach((val, index) => {
    if (index === 0) {
      prev = val;
      return;
    }

    result.push(prev - val);
    prev = val;
  });

  const prep = result
    .map((diff) => {
      const abs = Math.abs(diff);
      if (!(abs > 0 && abs <= 3)) {
        return 0;
      }

      if (direction > 0) {
        return +(diff > 0);
      }
      return +(diff < 0);
    });

  return { result, prep };
}
